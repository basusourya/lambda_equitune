...
from utils import initialize_equi_model, set_seed, print_details
from train import train_model
from custom_transforms import RandomRot90


def create_dataloaders(args, input_size, data_dir, batch_size, num_workers=0):
    # Transformations
    ...
    if args.dataset == "Hymenoptera":
        ...
        dataloaders_dict = {
            x: torch.utils.data.DataLoader(image_datasets[x], batch_size=batch_size, shuffle=True, num_workers=num_workers) for x in
            ['train', 'val']}
...
def plot_distance(equitune_distance_list, val_acc_list):
    num_epochs = len(equitune_distance_list)
    epochs = [i for i in range(num_epochs)]
    equitune_distance_list = [equitune_distance_list[i].cpu() for i in range(len(equitune_distance_list))]
    val_acc_list = [val_acc_list[i].cpu() for i in range(len(val_acc_list))]
    ...
    plt.figure(2)
    plt.plot(epochs, equitune_distance_list, label="Equi Dist")
    plt.legend()
    plt.savefig("equi_dist_plot.png")
...
def equitune(args):
    ...
    if not args.compute_distance:
        ...
        _, hist_equift, best_acc_equift, equitune_distance_list = train_model(model_equift, dataloaders_dict, criterion, optimizer_equift,
                                                      device,
                                                      num_epochs=args.num_epochs,
                                                      is_inception=(args.model_name == "inception"),
                                                      compute_distance=args.compute_distance)

        plot_distance(equitune_distance_list, hist_equift)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default="resnet", type=str, help="[resnet, alexnet, vgg, densenet]")
    ...
    parser.add_argument('-compute_distance', action='store_true',
                        help='compute distance between original features and equituned features for plots')
    ...
    # set compute distance to True
    args.compute_distance = True
    args.num_workers = 0
