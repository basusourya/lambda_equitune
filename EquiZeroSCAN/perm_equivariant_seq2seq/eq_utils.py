...
def get_scan_equivariance_indices(in_dim, out_dim, eq_word_indices, G, group_type='cyclic'):
    """
    :param vocab_size: size of the output vocab
    :param eq_word_indices: indices of the output words on which the cyclic group acts
    :param G: list of group elements
    :param group_type: cyclic by default
    :return: equivariance indices for designing equivariant linear networks
    """
    indices = [_ for _ in range(in_dim * out_dim)]  # initialize orbits before parameter sharing
    v = np.zeros(in_dim * out_dim) + 1  # -1 if not visited, else 1
    current_orbit = len(indices) - 1
    ...
    if group_type == 'permutation':
        ...
        diagonal = np.ones(in_dim)
        indices = torch.tensor(np.diag(diagonal)).long().view(in_dim * out_dim).tolist()
        return squeeze_orbits(indices)

    if in_dim == 1:
        in_eq_word_indices = range(1)
    else:
        in_eq_word_indices = eq_word_indices

    for i in in_eq_word_indices:
        for j in eq_word_indices:
            index = i * out_dim + j
            v[index] = -1
            indices[index] = -1
            current_orbit -= 1
    ...
    for i in in_eq_word_indices:
        for j in eq_word_indices:
            ...
            if v[index] < 0:
                current_orbit += 1
                # iterate over the entire group to find edges that belong to the group
                # and assign the same orbit number to each
                for g in G:
                    if in_dim == 1:
                        i_g, j_g = i, g[j]
                    else:
                        i_g, j_g = g[i], g[j]
                    index_g = i_g * out_dim + j_g
                    v[index_g] = 1
                    indices[index_g] = current_orbit
