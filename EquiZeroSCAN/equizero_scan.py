...
Output: Equizeroed model

Test: Use equizeroed model, but also use the output transform used in equizero.
"""
import random
import argparse
import os
...
from perm_equivariant_seq2seq.g_utils import cyclic_group_generator, cyclic_group, g_transform_data,\
    g_inv_transform_prob_data, min_loss_index
...
parser.add_argument('--loss_func_name',
                    choices=['entropy', 'top1'],
                    default='top1',
                    help='loss function used for EquiZero')
...
parser.add_argument('--save_dir',
                    type=str,
                    default='./equizero_models/',
                    help='Top-level directory for saving equizero experiment')
...
parser.add_argument('--compute_zero_shot_accuracy',
                    dest='compute_zero_shot_accuracy',
                    default=True,
                    action='store_true',
                    help="Boolean to compute zero shot accuracy")
...
def test_accuracy(model_to_test, pairs, in_G, out_G, loss_func_name='entropy'):
    ...
    with torch.no_grad():
        for pair in pairs:
            ...
            outputs = g_inv_transform_prob_data(transformed_outputs, G=out_G)  # dim [group_size, max_sentence_size=50, vocab_size=8]
            equizero_index = min_loss_index(outputs,
                                            loss_func_name=loss_func_name)  # dim [1] gives the group element to choose
...
if __name__ == '__main__':
    ...
    equizero_model.to(device)

    # Initialize optimizers
    encoder_optimizer = torch.optim.Adam(list(model.encoder.parameters()) + list(equizero_model.parameters()),
    ...
    if args.n_iters == 0:
        ...
        if args.validation_size > 0.:
            val_acc = test_accuracy(model, validation_pairs, in_G, out_G, loss_func_name=args.loss_func_name).item()
            val_accuracies.append(val_acc)
            print('Zero shot validation accuracy %s' % (val_acc))

        if args.compute_test_accuracy:
            test_acc = test_accuracy(model, testing_pairs, in_G, out_G, loss_func_name=args.loss_func_name)
    ...
    for iteration in range(0, args.n_iters):
        ...
        loss = equizero(input_tensor=iteration_input,
                        target_tensor=iteration_output,
                        model_to_train=equizero_model,
                        in_G=in_G,
                        out_G=out_G,
